...
| `/character` | Sybillian-style Storyteller broker. | `melius-commands/commands/character.json` | Lets the Storyteller role-display UI call Sybillian's character display helper and sync Jay's reveal data. |
| `/botc grimoire confirm`, `change_characters`, `edit_seat`, `set_character`, `set_good`, `set_evil` | Pre-reveal Storyteller editor. | `melius-commands/commands/botc.json`, `botc_patch:grim/confirm`, `botc_patch:grim/editor/*` | Uses Sybillian's stored seat names and current script through guarded server-side dialogs. Role changes update cached reveal state and the acting Storyteller display; alignment overrides remain reveal-only. Every edit path locks when the Storyteller confirms and starts Reveal Grimoire. |
...
| Teleports | `ct:cmd/tpchurch`, `ct:cmd/tpallhome`, Sybillian house markers | `/st tphouse`, `/tpchurch`, `/tpallhome`, manual `/botc` compatibility fallbacks |
...
| Role display toggles | FancyMenu `p<seat>_role` variables and `ct:cmd/character` | `/character` remains guarded by `tag=storyteller`; the Reveal Grimoire editor directly updates only the acting Storyteller's matching FancyMenu variable without changing the seated player's gameplay role/tags |
...
`/tpchurch`, `/tpallhome`, `/request_chat`, and `/character` broker commands.
...
Run `tools/tests/audit-fancymenu-actions.ps1` before changing FancyMenu button
actions. It scans Jay's Patch FancyMenu sources and fails if a menu button sends
a protected raw command instead of going through a guarded bridge command.

Run `tools/tests/audit-fancymenu-setupbag-bursts.ps1` before changing setup-bag
FancyMenu buttons. Run `tools/tests/test-setupbag-burst-bridges.ps1` for a stronger
gate that requires one-command `/setupbag ...` bridges for known setup-bag
targets and explicitly fails `/botc setup...` menu actions.
Keep setup flows in sync with
`docs/project-notes/setupbag-one-command-bridge-plan.md` and
`tools/fancymenu-menu-command-policy.json` as bridge coverage evolves.
