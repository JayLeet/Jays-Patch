# Let the Storyteller confirm the Banshee's Demon-caused death; the server cannot infer that cause reliably.
dialog clear @s
execute unless entity @s[tag=storyteller] run return 0
execute unless score phase game_data matches 4 run return 0
execute unless entity @a[tag=!storyteller,tag=!spectator,tag=dead,tag=!active_banshee,scores={id=1..15,role=55}] run return 0
...
tag @a[tag=!storyteller,tag=!spectator,tag=dead,tag=!active_banshee,scores={id=1..15,role=55}] add botc_banshee_newly_active
