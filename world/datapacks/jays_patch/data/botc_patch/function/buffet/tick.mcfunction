...
execute if score buffet_mode botc_patch matches 1..2 as @a[tag=botc_buffet_roster,tag=!storyteller,scores={botc_hand_use=1..},tag=!botc_buffet_action_used] if data entity @s SelectedItem.components."minecraft:custom_data"{botc_buffet_personal_grimoire:1b} run function botc_patch:buffet/personal_grimoire/open
execute if score buffet_mode botc_patch matches 1..2 as @a[tag=botc_buffet_roster,tag=!storyteller,scores={botc_hand_use=1..}] if data entity @s SelectedItem.components."minecraft:custom_data"{botc_buffet_personal_grimoire:1b} run tag @s add botc_buffet_action_used
...
execute if score phase game_data matches 0 if score buffet_mode botc_patch matches 2 as @a[tag=botc_buffet_roster,tag=!storyteller,scores={botc_hand_use=1..},tag=!botc_buffet_action_used] if items entity @s weapon.offhand minecraft:carrot_on_a_stick[minecraft:custom_data~{botc_buffet_choices:1b}] run function botc_patch:buffet/draft/open_choices
...
execute if score phase game_data matches 0 if score buffet_mode botc_patch matches 2 as @a[tag=botc_buffet_roster,tag=!storyteller,scores={botc_hand_use=1..}] if items entity @s weapon.offhand minecraft:carrot_on_a_stick[minecraft:custom_data~{botc_buffet_choices:1b}] run tag @s add botc_buffet_action_used
