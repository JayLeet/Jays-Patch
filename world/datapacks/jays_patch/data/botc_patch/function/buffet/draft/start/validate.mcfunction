...
execute unless score buffet_hard_valid botc_patch matches 1 run tellraw @s [{"text":"! ","color":"red","bold":true},{"text":"Draft cannot start: every player in the draft must be online and finished, every setup choice must be complete, and the final character counts must be legal.","color":"gray","bold":false}]
