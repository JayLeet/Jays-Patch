...
# Executes and kills the alive player in seat 2 for broken Cerenovus madness.
...
execute unless entity @a[tag=!storyteller,tag=!spectator,tag=!dead,scores={id=2},limit=1] run return run tellraw @s [{text:"! ",color:"red",bold:true},{text:"That player is no longer available to execute.",color:"gray",bold:false}]
...
execute as @a[tag=!storyteller,tag=!spectator,tag=!dead,scores={id=2},limit=1] run function ct:kill/execute/mark
tag @a[tag=!storyteller,tag=!spectator,tag=!dead,scores={id=2},limit=1] add botc_st_last_executed
execute if score @a[tag=!storyteller,tag=!spectator,tag=!dead,scores={id=2},limit=1] role matches 107 run return run function botc_patch:storyteller_tools/boomdandy/prepare_execution_choice
execute as @a[tag=!storyteller,tag=!spectator,tag=!dead,scores={id=2},limit=1] run function ct:kill/execute/execute
