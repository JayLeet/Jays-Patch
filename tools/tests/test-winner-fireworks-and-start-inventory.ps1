...
foreach ($entry in @(
    ...
    $giveLines = @($entry.Text -split "`r?`n" | Where-Object { $_ -match '^give @s minecraft:firework_rocket' })
    if ($giveLines.Count -ne 5) {
        throw "Broken winner/start safety invariant: $($entry.Name) must receive exactly five unique rockets, found $($giveLines.Count)"
    }

    $shapes = @($giveLines | ForEach-Object {
    ...
    Assert-Contains $entry.Text 'colors:\[I;[^\]]+\],fade_colors:\[I;[^\]]+\]' "$($entry.Name) color-to-fade gradients"
...
Assert-Contains $clearStale 'clear @s minecraft:firework_rocket\[minecraft:custom_data=\{botc_patch_winner_firework:1b\}\]' "marker-only carried reward cleanup"
...
Assert-Contains $cleanupDropped 'if items entity @s contents minecraft:firework_rocket\[minecraft:custom_data=\{botc_patch_winner_firework:1b\}\] run kill @s' "marker-only dropped reward cleanup"
