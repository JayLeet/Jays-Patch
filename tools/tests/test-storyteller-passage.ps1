...
Assert-Contains $night 'tag=storyteller,tag=!in_house\] at @s unless block ~ -64 ~ minecraft:acacia_planks run effect give @s minecraft:invisibility 3 0 true' "night invisibility outside houses and church"
Assert-Contains $night 'tag=storyteller\] at @s if block ~ -64 ~ minecraft:acacia_planks run effect clear @s minecraft:invisibility' "night church visibility"
Assert-NotContains $night '#ct:vc_marker' "night visibility avoids broad voice-zone matching"
...
Assert-Contains $resetStoryteller 'scoreboard players operation @s botc_st_gen = storyteller_generation botc_patch' "Storyteller generation preservation during setup reset"
Assert-NotContains $resetStoryteller 'scoreboard players reset @s botc_st_gen' "Storyteller generation removal during ownership-preserving reset"
